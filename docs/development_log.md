...
**작업 내용**
- 정해둔 3대 델타 외에 앱·논문 강화 아이디어를 전 방면으로 브레인스토밍.
- `docs/enhancement_ideas.md` 신설: A모델/B신호/C앱UX·XAI/D센서융합/E평가/F데이터/G배포 + TOP6 + 조합 시나리오.

**TOP 6 추천**
1. 카메라+마이크 멀티모달(음향으로 fps 한계 보완) 2. XAI+결함주파수 오버레이 3. 불확실성 정량화+방어 UX
4. 실시간 촬영 품질 가이드 5. 공개 스마트폰 베어링 데이터셋 6. 입력표현·모델 벤치마크(ANN 비교 대체)

**주목**
- D1(음향 융합)·A3(physics-informed)·D3(롤링셔터)는 fps 최대 약점을 정면 돌파하는 방향.
- C1/C2(XAI·결함주파수 오버레이)는 포스터 그림4를 앱으로 가져오는 확장.

**수정 파일**
- `docs/enhancement_ideas.md` (신규)
- `docs/paper_concept.md` (링크 추가)
- `docs/development_log.md`

---

## 2026-08-03 — 포스터 대비 델타 정리 (노블티 = 증분 관점)

**작업 내용**
- 포스터(1009.pdf) 전문 재확인 후, "저널화 = 포스터 대비 무엇을 강화하나" 관점으로 노블티 재정의.
- `paper_concept.md`에 "1-A. 포스터 대비 델타" 섹션 신설.
- 스마트폰 마커리스 사례 조사 반영(Orak 2018: Note8 240fps 마커리스 캔틸레버 = 저주파만; 베어링은 빈 자리).

**핵심 정리**
- 포스터가 이미 한 것(델타 아님): 앱, WDCNN 99.7%, FFT 20Hz 일치, ANN 비교. 마커리스·강건성은 포스터가 스스로 "향후연구"로 명시.
- 저널 델타 우선순위: ① 99.7% 타당성(누수 재평가, 포스터 무검증) ② 그림4 확장(회전20Hz→결함주파수 BPFO/BPFI + 240fps Nyquist 경계) ③ 다조건 강건성+불확도 (보조: 모델 심화/전이/XAI).
- 프레이밍: 포스터=PoC "만들었다" → 저널=엄밀검증 "어디까지·얼마나 정직하게 되는가 규명".
- 가장 날카로운 한 방: "포스터 그림4를 완성한다"(결함주파수·fps경계) + 누수 없는 정직한 성능.

**수정 파일**
- `docs/paper_concept.md` (1-A 델타 섹션 신설, Option A 마커리스 실현가능성)
- `docs/related_works.md` (Orak 캔틸레버 정독, 스마트폰 마커리스 조사)
- `docs/experiment_design.md` (실험2-B를 그림4 확장으로 연결)
- `docs/development_log.md`
...
## 2026-07-23 — Tier1 3편 정독 + 실험 설계서 작성 (문헌→실험 전환)

**작업 내용**
- Tier1 3편 정독: D6(Few-shot Siamese-WDCNN, Sci.Rep 2025), C4(불확도 메트롤로지 리뷰, Buildings 2026), B5(멀티모달 LLM, Sensors 2025).
- `docs/experiment_design.md` 신규: 문헌 종합 기반 실험 설계서(실험0~5 + 우선순위 + 스토리라인).

**핵심 발견**
- D6: **우리 소규모데이터 해법** = CWRU 사전학습→few-shot 전이. cold-start 정확도 WDCNN 70%→FSL+TL 85%→+AM 88.75%(정직한 수치). "저속일수록 결함특징 증폭".
- C4: **정직한 평가 축 methods 골격** = 영상변위법을 측정시스템(측정모델+불확도예산+타당성조건+참조비교)으로. 우리는 Level0(scale factor) 매핑. "저샘플링 미분→노이즈 증폭"으로 fps 논지 재확인.
- B5: 이벤트카메라+Qwen2.5-VL-7B 95.4%지만 특수HW+7B LLM 무거움 → 스마트폰+경량 WDCNN 배포성 우위(novelty 재확인).

**실험 설계서 요지**
- 실험1(누수 재평가, 최우선): 분할 S0~S3 곡선 + Macro AUROC.
- 실험2(fps 유효범위): 결함주파수 vs Nyquist 경계표 + DisplacementZ FFT 기여도.
- 실험3(다조건 강건성): RPM·조명·거리·각도·기기·흔들림 통제실험(C2 프로토콜+C4 불확도예산).
- 실험4(소규모/일반화): CWRU 전이+Siamese-WDCNN+복수베어링.
- 실험5(앱 확장): 메타데이터 로깅·자이로보정·마커리스(코드작업 승인필요).

**생성/수정 파일**
- `docs/experiment_design.md` (신규)
- `docs/related_works.md` (D6/C4/B5 정독 반영)
- `docs/development_log.md`

**정독 누계**: 19편 (54편 중). 문헌조사 1차 마무리 → 실험 설계 단계 진입.
...
**방향 조정**
- 사용자 지적: 누수 분석에 과몰입 말 것. 목표는 **앱 확장 + 논문화**. 누수는 비판이 아니라 기여로 흡수.

**작업 내용**
- SURVISHNO 2019 contest (MSSP 2021) 정독 — 스마트폰 진동분석 novelty 축의 핵심 근거.
- `docs/paper_concept.md` 신규: 건설적 종합(한 문장 컨셉, novelty 위치, 앱 확장, 기여 후보, 로드맵).

**핵심 발견**
- SURVISHNO 주장이 우리와 동일: "주머니 속 스마트폰 = 저비용 진동 DAQ". 단 IAS·모달분석이지 결함 분류 아님.
- → **접근성 서사는 top 저널(MSSP)에서 이미 인정된 가치.** 우리 novelty("스마트폰+베어링 결함분류+온디바이스")의 gap 재확인.
- 논문 두 축 확정: 건설적(스마트폰 접근성+온디바이스+강건성) 主 / 신뢰성(누수없는 평가) 補→기여.

**생성/수정 파일**
- `docs/paper_concept.md` (신규)
- `docs/related_works.md` (SURVISHNO 정독 반영)
- `docs/development_log.md`

**정독 누계**: 8편 (E1, E2, TICNN, Peng, C2, WDCNN원논문, 이벤트카메라, SURVISHNO).
...
**작업 내용**
- 라이너 스콜라 조사 결과(30편, 블록 A~E 분류) CSV를 `papers/liner_paper_list.csv`로 이관.
- Claude WebSearch로 CSV 미포함 논문(ScienceDirect 영상진동 계열, 스마트폰 SHM/스포츠 계열) 추가 조사.
- `docs/related_works.md`를 30+편 기준으로 전면 재작성: 블록 A(마커리스)/B(카메라진단)/C(촬영강건성)/D(도메인적응·WDCNN)/E(데이터누수) + 웹검색 보강 섹션 + 신규성 포지셔닝.
- `docs/research_gaps_and_limitations.md`에 조사 문헌 근거를 각 한계 항목에 연결.
- PDF/엑셀 정리: 루트 `.gitignore`에 `*.pdf`, 논문은 `papers/`로 집결.

**핵심 발견 (문헌 근거)**
- 데이터 누수(블록 E): *Impact of Data Leakage in Vibration Signals* (IEEE Access 2024) — 분할 방식만 바꿔도 정확도 40%+ 하락. 우리 99.7%(슬라이딩 윈도우+10-fold)의 재평가가 최우선 과제로 확정.
- WDCNN 원논문 계보 확인: 본 논문 [2](Zhang, Sensors 2017)의 자매편 *MSSP 2018*(1033회 인용) 확보 필요.
- 신규성: 영상 기반 베어링 진단 선행은 대부분 산업용 고속·이벤트 카메라. **소비자 스마트폰 슬로모션 + 베어링 분류 + 온디바이스 DL** 교집합은 희소 → 우리 위치.
- 촬영 강건성 평가 프로토콜 벤치마크 확보(블록 C: 조명·각도·저조도).

**생성/수정 파일**
- `papers/liner_paper_list.csv` (이동), `papers/README.md`
- `.gitignore` (신규, 루트)
- `docs/related_works.md` (전면 개정)
- `docs/research_gaps_and_limitations.md` (문헌 근거 보강)
- `docs/development_log.md`

---

## 2026-07-23 — 논문 한계 분석 및 학술 저널 확장 방향 정리

**작업 내용**
- 정보공학회 포스터 논문("스마트폰 카메라를 활용한 비접촉식 지능형 회전기계 결함 진단 시스템") PDF를 검토.
- 기존 접촉식 센서 방식 및 카메라 기반 비접촉 방식(LDV, DIC, 광학 흐름)의 한계를 정리.
- 본 연구의 자체 한계를 교수님 제시 분류(마커 의존성 / 정확도 신뢰성 / 촬영 환경 의존성 / 연구 수준) 기준으로 세부 분석.
- 학술 저널 논문으로 확장하기 위한 5개 방향(마커리스, 다조건 강건성, 도메인 적응, XAI, 공개 데이터 비교) 도출.
- 조사 필요 관련 논문 목록 및 보완 실험 항목 정리.

**생성 파일**
- `docs/research_gaps_and_limitations.md` — 한계 분석, 논문 확장 방향, 관련 논문 목록

**주요 확인 사항**
- 본 논문의 가장 큰 취약점: 단일 조건(1200 RPM, 연속광, iPhone 12 Pro, 고정 삼각대) + 독립 테스트셋 없음
- 99.7% 정확도는 동질 데이터 10-fold CV 결과로, 독립 검증이나 다조건 검증이 없으면 저널 리뷰어 지적 예상
- 마커리스 접근 또는 다조건 강건성 평가가 저널급 기여의 핵심 후보
- Android/iOS 간 IR→OR 오분류 1건은 논문 신뢰성을 위해 반드시 원인 규명 필요
