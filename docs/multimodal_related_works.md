...
3. **저주파 영상 + 고주파 음향의 주파수 분할 융합은 미개척.** "영상=저주파 변위 / 음향=고주파 결함 임펄스"를 베어링 진단에 명시적으로 제안한 연구 없음. Samuelson 2020 [4](SHM)이 가장 근접.
4. **스마트폰 슬로모션 영상의 오디오 트랙 활용은 Lu 2016 [1]이 유일.** 단 (a) 영상에서 변위 스펙트럼이 아닌 **회전속도(IFR)만** 추출, (b) **FCO=IFCF/IFR 나눗셈**이라는 단순 조합, (c) **딥러닝 특징 융합 미사용**.
...
### 2-1. 카메라 + 마이크 동시 (우리와 직접 경쟁) — 단 3편
| 논문 | 카메라 | 융합 방식 | 스마트폰? | 우리와 차이 |
|------|--------|-----------|:--------:|-------------|
| **Lu 2016 [1]** (Math. Prob. Eng., 6회) | **스마트폰** | 신호처리 FCO=IFCF/IFR | ✅ | 영상=회전속도만(변위X), 나눗셈 조합, DL 없음 |
...
### 2-3. 음향-진동 융합 딥러닝 (가속도계+마이크, 영상 없음 — 융합 아키텍처 참고용)
| 논문 | 기여 |
|------|------|
