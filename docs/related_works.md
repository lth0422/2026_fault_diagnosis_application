...
| C2 | Video-Based Micro-Vibration Measurement for Hydraulic Structures in Field Environments | Earthq. Eng. Resil., 2025 | 0 | **조명·촬영각도·스케일** 변화 강건성. 센서 상관 0.96. **균일조명 필수, 직각 촬영 중요** (실용 가이드라인) |
...
| D3 | **A deep CNN with new training methods for bearing fault diagnosis under noisy environment and different working load** | MSSP, 2018 | **1033** | **WDCNN(넓은 첫 커널) 원논문.** 원시신호 입력, 잡음/부하변화 강건, 특징 시각화 |
...
> 참고: 우리 논문의 참고문헌 [2]는 Zhang et al., *Sensors* 2017 (WDCNN, anti-noise+domain adaptation). #D3은 같은 저자군의 *MSSP* 2018 자매 논문. 두 편 모두 인용 근거로 확보 권장.
...
| # | 제목 | 출처 | 인용 | 핵심 메시지 |
|---|------|------|-----:|------|
| E1 | **Impact of Data Leakage in Vibration Signals Used for Bearing Fault Diagnosis** | IEEE Access, 2024 | 5 | 분할 방식만 바꿔도 **정확도 40%+ 하락**. 선행 55편 중 10편이 누수 의심, 6편만 제대로 처리 |
| E2 | Towards a more realistic evaluation of ML models for bearing fault diagnosis | arXiv, 2025 | 0 | **bearing-wise 분할** 제안. segment/condition 분할은 과도낙관. 학습 베어링 다양성이 일반화의 결정 요인 |
...
| 제목 | 출처 | 관련 블록 | 비고 |
|------|------|-----------|------|
| A visual vibration characterization method for intelligent fault diagnosis of rotating machinery | MSSP, 2023 | A/B | **우리와 구조 최유사**: 이미지 위상차→그레이스케일 인코딩→CNN, 질량로딩 없음 |
